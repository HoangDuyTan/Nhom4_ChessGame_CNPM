...
public class Board {
    ...
    public boolean move(Position from, Position to, String promotionChoice) {
        ...
        if (piece == null || !isLegalMove(from, to, piece)) {
            return false;
        }
        ...
        if (piece instanceof Pawn && isPromotionRank(piece, to)) {
    ...
    public boolean canCaptureEnPassant(Position from, Position to, Color color) {
        ...
        Piece capturedPawn = capturedPawnPosition.isValid() ? get(capturedPawnPosition) : null;
                ...
                if (p != null && p.getColor().equals(color)) {
                    for (int r2 = 0; r2 < 8; r2++) {
                        for (int c2 = 0; c2 < 8; c2++) {
                            if (isLegalMove(from, new Position(r2, c2))) {
                                return true;
                            }
                        }
                    }
